...
from tools_gen import retrieve
...
try:
    ...
    print("⚠️ 필수 파일(utils.py, models.py)이 없습니다.")
    exit()
...
def supervisor(state: State): # supervisor 에이전트
    ...
    # 메시지 추가
    supervisor_message = AIMessage(content=f"[Supervisor] {task}")
...
def content_strategist(state: State):
    ...
    if task_history[-1].agent != "content_strategist": 
        # 단순 경고로 처리하거나 예외 발생
        print(f"Warning: Content Strategist가 아닌 agent({task_history[-1].agent})가 작업을 수행했습니다.")
    
    task_history[-1].done = True
    task_history[-1].done_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    # 다음 작업이 communicator로 고정되어 있으므로 추가
    new_task = Task(
        agent="communicator",
        done=False,
...
def communicator(state: State):
    ...
    if task_history[-1].agent != "communicator":
         print(f"Warning: Communicator가 아닌 agent({task_history[-1].agent})가 작업을 수행했습니다.")
...
graph_builder.add_node("vector_search_agent", "communicator")
